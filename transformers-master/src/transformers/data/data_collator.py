...
class DataCollatorForLanguageModeling:
    ...
    replaced_token_percentage: float = 0.1
    kept_token_percentage: float = 0.1
    masked_token_percentage: float = 0.8
    pad_to_multiple_of: Optional[int] = None
    ...
    def mask_tokens(
        ...
        indices_replaced = torch.bernoulli(torch.full(labels.shape, self.masked_token_percentage)).bool() & masked_indices
        inputs[indices_replaced] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)

        # 10% of the time, we replace masked input tokens with random word
        #indices_random = torch.bernoulli(torch.full(labels.shape, 0.5)).bool() & masked_indices & ~indices_replaced
        relative_replaced_token_percentage = self.replaced_token_percentage / (1 - self.masked_token_percentage)
        indices_random = torch.bernoulli(torch.full(labels.shape, relative_replaced_token_percentage)).bool() & masked_indices & ~indices_replaced
